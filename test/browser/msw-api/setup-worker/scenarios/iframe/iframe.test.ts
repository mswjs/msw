...
test.fixme(
  'intercepts a request from an iframe (nested client)',
  async ({ loadExample, page }) => {
    ...
    const firstName = await firstNameElement.evaluate(
...
test.fixme(
  'intercepts a request from a deeply nested iframe',
  async ({ loadExample, page }) => {
    await loadExample(require.resolve('./iframe.mocks.ts'), {
...
test.fixme(
  'intercepts a request from a deeply nested iframe given MSW is registered in a parent nested iframe',
  async ({ webpackServer, loadExample, page }) => {
    await loadExample(require.resolve('./iframe.mocks.ts'), {
      markup: path.resolve(__dirname, 'page-in-iframe.html'),
      beforeNavigation(compilation) {
        compilation.use(staticMiddleware)
...
test.fixme(
  'intercepts a request from an iframe given MSW is registered in a sibling iframe',
  async ({ webpackServer, loadExample, page, context }) => {

...
interface BrowserRuntime {
  ...
  sse: typeof import('msw').sse
  ws: typeof import('msw/ws').ws
}

export function inlineModule(
  setup: (runtime: BrowserRuntime) => Promise<void> | void,
): CompilationEntry {
  return {
    source: `
      import { bypass, http, HttpResponse, passthrough, sse } from 'msw'
...
export function inlineSource(source: string): CompilationEntry {
  return { source }
}
...
export const test = base.extend<TestFixtures, WorkerFixtures>({
  ...
  viteServer: [
    async ({}, use) => {
      const viteServer = await createViteHttpServer()
      await use(viteServer)
      await viteServer.close()
    },
    { scope: 'worker' },
  ],
  async loadExample({ page, viteServer, waitForMswActivation }, use) {
    ...
    await use(async (entry, options = {}) => {
      ...
      compilation = await viteServer.compile(entries, options)
  ...
  async makeUrl({ viteServer }, use) {
    await use((pathname) => {
      return new URL(pathname, viteServer.serverUrl).href
    })
  },
  async spyOnConsole({ page }, use) {
    ...
    await use((customPage) => {
      messages = spyOnConsole(customPage || page)
      return messages
    })
